...
contract OflightsFarm is Ownable {
    using SafeERC20 for IERC20;
    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many LP tokens the user has provided.
        uint256 rewardDebt; // Reward debt. See explanation below.
        uint256 remainingOflyTokenReward;  // OFLY Tokens that weren't distributed for user per pool.
        //
        // We do some fancy math here. Basically, any point in time, the amount of OFLY
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accOFLYPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws Staked tokens to a pool. Here's what happens:
        //   1. The pool's `accOFLYPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accOflyPerShare; // Accumulated OFLY per share, times 1e12. See below.
        uint32 lastRewardTime; // Last timestamp number that OFLY distribution occurs.
        uint16 allocPoint; // How many allocation points assigned to this pool. OFLY to distribute per second.
        
        
    }
    
    IERC20 immutable public ofly; // The OFLY TOKEN!!
    
    uint256 public oflyPerSecond; // Ofly tokens vested per second.
...
// How many pools are in the contract
    ...
    // Update the given pool's OFLY allocation point. Can only be called by the owner.
    // Good practofly to update pools without messing up the contract
    function set(
        uint256 _pid,
        uint16 _allocPoint,
    ...
    // Deposit staking tokens to OflightsFarm for OFLY allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            user.remainingOflyTokenReward = safeRewardTransfer(msg.sender, pending);
        }
        ...
        user.rewardDebt = user.amount * pool.accOflyPerShare / 1e12;
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw staked tokens from OflightsFarm.
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        user.remainingOflyTokenReward = 0;
        pool.stakingToken.safeTransfer(address(msg.sender), userAmount);
        emit EmergencyWithdraw(msg.sender, _pid, userAmount);
    }

    // Safe ofly transfer function. Just in case if the pool does not have enough OFLY token,
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns(uint256) {
        uint256 oflyTokenBalance = ofly.balanceOf(address(this));
        if (oflyTokenBalance == 0) { //save some gas fee
            return _amount;
        }
        if (_amount > oflyTokenBalance) { //save some gas fee
            ofly.safeTransfer(_to, oflyTokenBalance);
            return _amount - oflyTokenBalance;
        }
        ofly.safeTransfer(_to, _amount);
        return 0;
    }
}
...
contract OFlightsFarm is Ownable {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accOflyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accOflyPerShare; // Accumulated SUSHIs per share, times 1e12. See below.
    }
    // The OFLY TOKEN!
    ...
    // The block number when OFLY mining starts.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accOflyPerShare = accOflyPerShare.add(
                sushiReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accOflyPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Deposit LP tokens to MasterChef for OFLY allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accOflyPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            safeOflyTransfer(msg.sender, pending);
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accOflyPerShare).div(1e12);
